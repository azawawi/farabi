...
$(function() {
	...
	var ping = function() {
		$.post("/ping", {}, function() {
			...
			if(firstTime) {
				...
				window.changeMode(diffEditor, 'diff', 'text/x-diff');
	...
	var tabId = 1;
	window.AddEditorTab = function(tabName, filename, mode, mimeType, contents) {
		...
		if( !mode ) {
			...
			window.changeMode(editor, 'perl', 'text/x-perl');
		} else {
			window.changeMode(editor, mode, mimeType);
		}
	...
	window.changeMode = function(cm, mode, mimeType) {
		CodeMirror.modeURL = "assets/codemirror-v4.8/mode/%N/%N.js";
		cm.setOption("mode", mimeType);
	...
	$(document).on('action-perl-tidy', function() {
		...
		$.post('perl_tidy', {"source": source}, function(result) {
			...
			if (result.stderr.length) {
				// Display Perl::Tidy error
				outputEditor.setValue("Perl::Tidy Error:\n" + result.stderr + "\n");
			} else {
				// Save current cursor position
				var cursor = editor.getCursor();

				// Set tidied text to editor
				editor.setValue(result.stdout);
	...
	$(document).on('action-project-build', function() {
		$.post('/project', {cmd: 'build'}, showCmdOutput);
	});

	$(document).on('action-project-test', function() {
		$.post('/project', {cmd: 'test'}, showCmdOutput);
	});

	$(document).on('action-project-clean', function() {
