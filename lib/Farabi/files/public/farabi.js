...
$(function() {
	...
	var showEditorStats = function(cm) {
		...
		$editorStats.html(
			'<span class="badge badge-info"><strong>' + line_number + '</strong>' + plural(line_number) + ' line</span>' +
			'&nbsp;<span class="badge badge-info"><strong>' + column_number + '</strong>' + plural(column_number) + ' column</span>' +
			'&nbsp;<span class="badge badge-info"><strong>' + cm.lineCount() + '</strong>&nbsp;lines</span>' +
			(selection ?  '&nbsp;<span class="badge badge-info"><strong>' + selection.length +'</strong></span>&nbsp;selected characters' : '')  +
			'&nbsp;<span class="badge badge-info"><strong>' + cm.getValue().length + '</strong>&nbsp;characters</span>' +
			'&nbsp;<span class="badge badge-info"><strong>' + cm.lineCount() + '</strong>&nbsp;lines</span>&nbsp;&nbsp;' +
			((problemCount > 0) ? '<button id="problems-button" class="btn btn-danger"><strong>' + problemCount + '&nbsp;</strong>Problem(s)</button>' : '<span class="badge badge-success"><strong>Syntax Check OK</span>')
		);
	...
	var podChanged = function(cm) {
		cm.operation(function() {
			//cm.getValue()
			window.sendMessage('pod2html', {"source": ""}, function(html) {
				$('#pod').html(html);
			});
		});
	};
