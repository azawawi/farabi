...
function startFarabi(editorId) {
	...
	$("#action-name", $actionsDialog).typeahead({
		source : function(query, process) {
			$.ajax({
				type: 'POST',
				url: '/actions-typeahead',
				async: false,
				data: {'action-name': query},
	...
	$("#ok-button", $actionsDialog).click(function() {
		...
		var actionName = $("#action-name", $actionsDialog).val();
	...
	$(document).on('farabi-perl-tidy', function() {
	...
	$(document).on('farabi-open-file', function() {
	...
	$(document).on('farabi-open-url', function() {
	...
	$(document).on('farabi-syntax-check', function() {
		alert("TODO syntax check");
	});

	$(document).on('farabi-perl-critic', function() {
		alert("TODO perl critic");
	});

	$(document).on('farabi-options', function() {
		$optionsDialog.modal('show');
		$("#mode_selector", $optionsDialog).focus();
	});

	$(document).on('farabi-run', function() {
		// Run file
