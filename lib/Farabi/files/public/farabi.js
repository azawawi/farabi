...
$(function() {
	...
	var tabId = 1;
	window.AddEditorTab = function(tabName, filename, mode) {
		...
		editor.on("change", function(cm) {
				...
				timeoutId = setTimeout(function() {
					...
					var isClean = cm.isClean();
					if(isClean) {
						$("#myTab > li.active a").text(editors[currentEditorId].tabName);
					} else {
						$("#myTab > li.active a").text(editors[currentEditorId].tabName + " *");
					}
				}, 250);
	...
	$(document).on('action-close-file', function() {
		var editor = GetCurrentEditor();
		var editorId = $(editor.getTextArea()).attr("id");
		
		$("#myTab > li.active").remove();
		$("#myTabContent > div.active").remove();
		...
		currentEditorId = $(editor.getTextArea()).attr("id");	
	});

	$(document).on('action-close-files', function() {
		$("#myTab > li").remove();
	...
	$(document).on('action-save-file', function() {
		...
		$.post('/save-file', {"filename": filename, "source": editor.getValue()}, function(data) {
			if(data.error == '') {
				alert("File saved!");
			} else {
				alert('Error:\n' + data.error);
