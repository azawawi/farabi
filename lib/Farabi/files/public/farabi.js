...
$(function() {
	...
	window.AddEditorTab = function(tabName, filename, mode, contents) {
		...
		var helpTimeoutId;
		var onCursorActivity = function(cm) {
				helpTimeoutId = setTimeout(function() {
					var mode = cm.getOption("mode");
					if (mode == "perl") {
						if($("#doc-tab").is(":visible")) {
							showHelp(cm);
						}
					}
				}, 250);
	...
	$('a[href="#doc-tab"]').click(function() { 
		showHelp(GetCurrentEditor());
	});
	...
	$(document).on('action-dzil-build', function() {
		$.post('/dzil', {cmd: 'build'}, showCmdOutput);
	});

	$(document).on('action-dzil-test', function() {
		$.post('/dzil', {cmd: 'test'}, showCmdOutput);
	});

	$(document).on('action-dzil-clean', function() {
		$.post('/dzil', {cmd: 'clean'}, showCmdOutput);
	});

	$(document).on('action-make', function() {
		$.post('/make', {cmd: ''}, showCmdOutput);
	});

	$(document).on('action-make-test', function() {
		$.post('/make', {cmd: 'test'}, showCmdOutput);
	});

	$(document).on('action-make-clean', function() {
		$.post('/make', {cmd: 'clean'}, showCmdOutput);
	});
