...
$(function() {
	...
	window.AddEditorTab = function(tabName, filename, mode, contents) {
		...
		var show_cmd_output = function(data) {
			...
			if(typeof data.stderr == "undefined") {
				...
				outputEditor.setValue("Invalid websocket response.. Please run it again. Hopefully will fix it soon :)");
	...
	var podChanged = function(editor) {
			//$('#pod').attr("src", "/preview?source=" + encodeURIComponent(editor.getValue()) + "&style=" + $("#pod_style_selector", $('#options-dialog')).val());
			...
			$.post("/preview", 
				{
					...
					var $iframe = $("#pod");
					var scrollTop = $iframe.parent().contents().scrollTop();
	...
	$(document).on('action-spell-check', function() {
		var editor = GetCurrentEditor();
		window.sendMessage('spell-check', {source: editor.getValue()}, function(result) {
			if(result.error) {
				editor.setValue(result.source);
			} else {
				alert("No result");
