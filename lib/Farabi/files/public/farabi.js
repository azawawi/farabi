...
$(function() {
	...
	$actionsDialog.keypress(function(e) {
		if(e.keyCode == 13) {
			$("#ok-button", $actionsDialog).click();
		}
	});

	$("#action-name", $actionsDialog).keyup(function(evt) {
	...
	/* REPL - Read-Eval-Read-Loop action */
	...
	var evalCommand = function() {
		$.ajax({
			...
			success: function(result) {
				...
				$("#output", $replDialog).val(oldOutput + result.out);
	...
	$("#command", $replDialog).click(function() {
		evalCommand();
	});
	
	$("#eval-button", $replDialog).click(function() {
		evalCommand();
	});
	
	/* End of REPL - Read-Eval-Read-Loop action */
