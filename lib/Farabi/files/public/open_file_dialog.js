...
$(function() {
	...
	var findFile = function() {
		...
		// Find this file
		$.post("/find_file", { "filename": filename }, function(results) {
	...
	$("#ok-button", $dialog).click(function() {
		...
		$("#matched-files option:selected", $dialog).each(function(key, value) { 
			...
			$.post(
				"/open_file", { "filename": filename }, function(result) {
					if(result.ok) {
						// Open the file in a new tab
						AddEditorTab(result.filename, filename, result.mode, result.mime_type, result.value);
					} else {
						// Error
						alert(result.value);
					}
